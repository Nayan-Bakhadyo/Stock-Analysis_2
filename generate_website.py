...
Reads xLSTM predictions from xlstm_predictions.json (generated by train_and_predict.py)
"""
...
class StockWebsiteGenerator:
    def __init__(self, json_file='analysis_results.json', 
                 xlstm_file='xlstm_predictions.json',
                 rl_signals_file='rl_results/rl_trading_signals_all.json', 
                 output_dir='stock_website'):
        self.json_file = json_file
        self.xlstm_file = xlstm_file
    ...
    def load_data(self):
        ...
        # Load xLSTM predictions from file (generated by train_and_predict.py)
        if os.path.exists(self.xlstm_file):
            with open(self.xlstm_file, 'r') as f:
                xlstm_json = json.load(f)
                self.ml_data = xlstm_json.get('stocks', {})
                last_updated = xlstm_json.get('last_updated', 'Unknown')
            print(f"✓ Loaded xLSTM predictions for {len(self.ml_data)} stocks from {self.xlstm_file}")
            print(f"  Last updated: {last_updated}")
        else:
            print(f"⚠ No xLSTM predictions file found at {self.xlstm_file}")
            print(f"   Run: python train_and_predict.py")
        
        # Load RL signals if available
        ...
        <div class="filters-section">
            ...
            <div class="filter-controls">
                ...
                <label style="display: inline-flex; align-items: center; gap: 0.5rem; padding: 0.5rem 1rem; background: var(--card-bg); border: 1px solid var(--border-color); border-radius: 0.5rem; cursor: pointer;">
                    ...
                    <span style="color: var(--text-primary); font-size: 0.875rem;">Has xLSTM Predictions</span>
                </label>
            </div>
        </div>
