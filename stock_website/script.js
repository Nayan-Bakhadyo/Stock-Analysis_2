...
function applyFilters() {
    ...
    if (filterMl) {
        filteredStocks = filteredStocks.filter(stock => {
            // Check if stock has xLSTM multi-horizon predictions
            if (stock.ml_predictions && stock.ml_predictions.multi_horizon) {
                return true;
            }
...
function createCompactCard(stock) {
    ...
    // Get prediction dates and xLSTM info
    let predictionDateStr = '';
    const mlPred = stock.ml_predictions;
    const rlDate = stock.rl_signal?.timestamp;
    
    if (mlPred || rlDate) {
        const dates = [];
        if (mlPred && mlPred.multi_horizon) {
            const mlDateFormatted = new Date(mlPred.last_updated).toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
            const strength = mlPred.model?.performance?.strength || 'N/A';
            const strengthColor = strength === 'STRONG' ? '#10b981' : strength === 'MODERATE' ? '#f59e0b' : '#6b7280';
            dates.push(`<span style="color: ${strengthColor};">xLSTM: ${strength}</span>`);
        } else if (mlPred?.last_updated) {
            const mlDateFormatted = new Date(mlPred.last_updated).toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
            dates.push(`ML: ${mlDateFormatted}`);
        }
        if (rlDate) {
            const rlDateFormatted = new Date(rlDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
...
function createMLPredictionsSection(mlPredictions) {
    ...
    const tradingSignal = mlPredictions.trading_signal || {};
    const model = mlPredictions.model || {};
    const isMultiHorizon = mlPredictions.multi_horizon === true;
    ...
    // Check if this is multi-horizon xLSTM format
    if (isMultiHorizon) {
        return createMultiHorizonPredictionsSection(mlPredictions);
    }
    
    // Legacy format: Create prediction cards for 7 days
...
function createMultiHorizonPredictionsSection(mlPredictions) {
    const predictions = mlPredictions.predictions;
    const horizonDetails = mlPredictions.horizon_details || {};
    const tradingSignal = mlPredictions.trading_signal || {};
    const model = mlPredictions.model || {};
    const performance = model.performance || {};
    
    // Horizons: t+1, t+3, t+5, t+10, t+15, t+20
    const horizons = predictions.horizons || [1, 3, 5, 10, 15, 20];
    
    // Create prediction cards for each horizon
    const predictionCards = horizons.map((h, index) => {
        const date = predictions.dates[index];
        const price = predictions.prices[index];
        const direction = predictions.directions[index];
        const confidence = predictions.confidences[index];
        const returnPct = predictions.returns[index];
        const lastPrice = model.last_actual_price;
        
        const changeClass = returnPct >= 0 ? 'positive' : 'negative';
        const dirIcon = direction === 'UP' ? '📈' : direction === 'DOWN' ? '📉' : '➡️';
        const dirColor = direction === 'UP' ? '#10b981' : direction === 'DOWN' ? '#ef4444' : '#6b7280';
        
        // Get horizon-specific metrics if available
        const horizonInfo = horizonDetails[h.toString()] || {};
        const horizonMape = horizonInfo.mape || performance.test_mape;
        const horizonDirAcc = horizonInfo.direction_accuracy || performance.avg_direction_accuracy;
        
        return `
            <div class="prediction-card" style="border-left: 3px solid ${dirColor};">
                <div class="prediction-horizon" style="font-weight: bold; color: #3b82f6;">t+${h}</div>
                <div style="font-size: 0.75rem; opacity: 0.7; margin-bottom: 0.5rem;">${date}</div>
                <div class="prediction-price" style="font-size: 1.2rem; font-weight: bold;">NPR ${price.toFixed(2)}</div>
                <div class="prediction-change ${changeClass}" style="display: flex; align-items: center; gap: 0.25rem; margin-top: 0.5rem;">
                    <span>${dirIcon}</span>
                    <span style="font-weight: 600; color: ${dirColor};">${direction}</span>
                </div>
                <div style="font-size: 0.8rem; margin-top: 0.25rem;">
                    <span class="${changeClass}">${returnPct >= 0 ? '+' : ''}${returnPct.toFixed(2)}%</span>
                </div>
                <div style="font-size: 0.7rem; opacity: 0.6; margin-top: 0.5rem; border-top: 1px solid rgba(0,0,0,0.1); padding-top: 0.5rem;">
    ...
    // Performance colors
    const mapeRating = performance.mape_rating || 'Fair';
    const mapeColor = mapeRating === 'Excellent' ? '#10b981' : 
                      mapeRating === 'Good' ? '#3b82f6' :
                      mapeRating === 'Fair' ? '#f59e0b' : '#ef4444';
    
    const signalStrength = performance.signal_strength || 'Moderate';
    const signalColor = signalStrength === 'Strong' ? '#10b981' :
                       signalStrength === 'Moderate' ? '#f59e0b' : '#ef4444';
    
    const strengthLabel = performance.strength || 'MODERATE';
    const strengthColor = strengthLabel === 'STRONG' ? '#10b981' :
                         strengthLabel === 'MODERATE' ? '#f59e0b' : '#ef4444';
    ...
    // Calculate flat days display
    const flatDays = tradingSignal.flat_days || 0;
    const daysInfo = flatDays > 0 
        ? `${tradingSignal.up_days}↑ ${tradingSignal.down_days}↓ ${flatDays}→`
        : `${tradingSignal.up_days} up, ${tradingSignal.down_days} down`;
    
    return `
        <div class="ml-predictions">
            <h3 class="section-title">🤖 Multi-Horizon AI Predictions (xLSTM Ensemble)</h3>
            ...
            <div class="trading-signal-box" style="background: linear-gradient(135deg, ${directionColor}20, ${directionColor}10); border-left: 4px solid ${directionColor}; padding: 1.5rem; border-radius: 8px; margin-bottom: 1.5rem;">
                <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem;">
                    <div>
                        ...
                        <div style="font-size: 0.9rem; opacity: 0.8; margin-top: 0.3rem;">
                            ${daysInfo} across 6 horizons
                        </div>
                    </div>
                    <div style="text-align: right;">
                        <div style="font-size: 0.75rem; opacity: 0.7;">Model Strength</div>
                        <div style="font-size: 1.2rem; font-weight: bold; color: ${strengthColor};">${strengthLabel}</div>
                    </div>
                </div>
            </div>
            
            <!-- Model Performance -->
            <div class="model-performance" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 1rem; margin-bottom: 1.5rem;">
                <div class="performance-metric" style="background: white; padding: 1rem; border-radius: 8px; border: 1px solid rgba(59, 130, 246, 0.2);">
                    <div style="font-size: 0.85rem; opacity: 0.7; color: #6b7280;">Avg MAPE</div>
                    <div style="font-size: 1.5rem; font-weight: bold; color: ${mapeColor};">${performance.test_mape.toFixed(2)}%</div>
                    <div style="font-size: 0.8rem; font-weight: 600; color: ${mapeColor};">${mapeRating}</div>
                </div>
                <div class="performance-metric" style="background: white; padding: 1rem; border-radius: 8px; border: 1px solid rgba(59, 130, 246, 0.2);">
                    <div style="font-size: 0.85rem; opacity: 0.7; color: #6b7280;">Direction Accuracy</div>
                    <div style="font-size: 1.5rem; font-weight: bold; color: ${signalColor};">${performance.avg_direction_accuracy.toFixed(1)}%</div>
                    <div style="font-size: 0.8rem; font-weight: 600; color: ${signalColor};">${signalStrength}</div>
                </div>
                <div class="performance-metric" style="background: white; padding: 1rem; border-radius: 8px; border: 1px solid rgba(59, 130, 246, 0.2);">
                    <div style="font-size: 0.85rem; opacity: 0.7; color: #6b7280;">Model Type</div>
                    <div style="font-size: 0.95rem; font-weight: bold; margin-top: 0.3rem; color: #1f2937;">xLSTM</div>
                    <div style="font-size: 0.75rem; opacity: 0.7; color: #6b7280;">${model.n_models} ensemble</div>
                </div>
                <div class="performance-metric" style="background: white; padding: 1rem; border-radius: 8px; border: 1px solid rgba(59, 130, 246, 0.2);">
                    <div style="font-size: 0.85rem; opacity: 0.7; color: #6b7280;">Current Price</div>
                    <div style="font-size: 1.3rem; font-weight: bold; color: #1f2937;">NPR ${model.last_actual_price.toFixed(2)}</div>
                    <div style="font-size: 0.75rem; opacity: 0.7; color: #6b7280;">${model.sector || 'Unknown'}</div>
                </div>
            </div>
            
            <!-- Multi-Horizon Predictions Grid -->
            <div style="margin-bottom: 1rem;">
                <div style="font-size: 0.9rem; font-weight: 600; margin-bottom: 0.75rem; color: #374151;">
                    📅 Predictions by Horizon (t+days)
                </div>
            </div>
            <div class="prediction-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(120px, 1fr)); gap: 0.75rem;">
                ${predictionCards}
            </div>
            
            <!-- Model Info -->
            <div style="margin-top: 1.5rem; padding: 1rem; background: rgba(59, 130, 246, 0.05); border-radius: 8px; font-size: 0.85rem;">
                <div style="margin-bottom: 0.5rem;"><strong>Model Details:</strong></div>
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 0.5rem;">
                    <div>• Architecture: xLSTM Multi-Horizon Ensemble</div>
                    <div>• Ensemble: ${model.n_models} models</div>
                    <div>• Features: ${model.n_features} indicators</div>
                    <div>• Lookback: ${model.lookback_days} days</div>
                </div>
                <div style="margin-top: 0.75rem; opacity: 0.7; font-size: 0.8rem;">
                    <strong>Horizons:</strong> t+1 (tomorrow), t+3, t+5, t+10, t+15, t+20 days ahead<br>
                    <strong>Metrics:</strong> MAPE < 2% = Excellent, 2-5% = Good, 5-10% = Fair | Direction > 70% = Strong
                </div>
            </div>
        </div>
    `;
