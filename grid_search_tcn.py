"""
Grid search for TCN architecture across multiple stocks and lookback periods
...
# Stocks to test
STOCKS = ['IGI', 'NABIL', 'NIB', 'ADBL', 'SBI', 'NBL', 'HBL', 'EBL', 'KBL', 'GBIME']
...
def generate_tcn_configs(lookback):
    """Generate grid search configurations for TCN"""
    configs = []
    
    # Test a subset of combinations to keep runtime reasonable
    # Focus on most promising combinations based on previous results
    
    for layers in TCN_CONFIGS['layers']:
        for units_1 in TCN_CONFIGS['units_1']:
            for dropout_1 in TCN_CONFIGS['dropout_1']:
                for lr in TCN_CONFIGS['learning_rate']:
                    for batch_size in TCN_CONFIGS['batch_size']:
                        for patience in TCN_CONFIGS['patience']:
                            ...
                            config = {
                                'name': f'tcn_lb{lookback}_l{layers}_u{units_1}_d{dropout_1}_lr{lr}_bs{batch_size}',
                                'lookback': lookback,
                                'layers': layers,
                                'units_1': units_1,
                                'units_2': units_2,
...
def main():
    print("\n" + "="*80)
    print("TCN GRID SEARCH - MULTIPLE STOCKS AND LOOKBACK PERIODS")
    print("="*80)
    print(f"Stocks to test: {len(STOCKS)}")
    print(f"Lookback periods: {LOOKBACK_PERIODS}")
    print(f"Configurations per lookback: ~{len(generate_tcn_configs(60))}")
    
    # Estimate total configurations
    total_configs = len(STOCKS) * len(LOOKBACK_PERIODS) * len(generate_tcn_configs(60))
    print(f"Total configurations to test: ~{total_configs}")
    print("="*80)
    
    # Load or create master log
    master_log_path = 'tcn_grid_search_master_log.csv'
    ...
    for symbol in STOCKS:
        ...
        try:
            ...
            for lookback in LOOKBACK_PERIODS:
                ...
                configs = generate_tcn_configs(lookback)
                print(f"Testing {len(configs)} TCN configurations...")
